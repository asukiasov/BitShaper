...
- [x] 4.7 (post-merge addition, user feedback round 2) Four fixes: (1) the seed field is no longer `required` and `generateFromForm` now auto-fills a random seed on blank submit, so *neither* Generate nor Randomize ever requires typing anything; (2) added a read-only shape-ID field + "Copy ID" button next to the preview (Clipboard API with an `execCommand` fallback), so the current ID is easy to copy for external/API use; (3) relabeled the catalog section "Curated marks" with a hint explaining these are named marks composed from primitives, to disambiguate from the primitive list itself; (4) each primitive-mix checkbox now shows a small live-rendered icon (a 1×1 `renderShape` of that primitive alone) next to its name, so primitives are recognizable visually before checking. Fixing this also caught a real bug: `renderCatalogView`'s `container.innerHTML = ""` was wiping out the new catalog heading/hint because both were being appended to the same element passed in — fixed by giving the catalog list its own child container. New/updated tests in `web/test/generator-form.test.ts` (blank-seed Generate, primitive-icon count).
