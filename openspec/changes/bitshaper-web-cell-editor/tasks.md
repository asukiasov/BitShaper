...
- [ ] 1.1 New module exporting `renderPrimitiveIcon(primitiveIndex: number, size = 24): string`
      — the body currently private in `web/src/primitive-usage.ts` (encode a 1×1 shape,
      `renderShape(id, { size })`).
- [ ] 1.2 `web/src/primitive-usage.ts`: delete the local function, import from
      `./primitive-icon.js`. No behaviour change.
- [ ] 1.3 `web/test/primitive-usage.test.ts`: fix the import if it referenced the internal;
      existing assertions still pass.

## 2. `web/src/shape-state.ts` — `replaceCell`

- [ ] 2.1 Add `replaceCell(shapeId, index, cell: CellDef): string`: decode (return input
...
- [ ] 2.2 `web/test/shape-state.test.ts`: round-trip (replace → decode → expected cells);
...
- [ ] 3.1 `buildCellEditor(container, { onEdit })` → `{ element, render(shapeId) }`. Vanilla
      DOM, structured like `web/src/ramp-panel.ts`. `element` is `div.cell-overlay-root`
      appended to `container`.
- [ ] 3.2 `render(shapeId)`: `decodeShapeId` guarded — on failure empty the root and return.
...
- [ ] 3.3 Click `.cell-hit`: select it, build `div.cell-popover` near the button
...
- [ ] 3.4 On any popover change: build `updated: CellDef = { type, rotation, invert }`,
      `next = cells.map((c,i) => i === index ? updated : c)`,
      `onEdit(encodeShapeId({ cols, rows, cells: next }))` — no ramp in the emitted ID.
- [ ] 3.5 Popover closes on `Esc`, outside `pointerdown`, or selecting a different cell.
- [ ] 3.6 During implementation, verify overlay alignment against real `renderShape` output
...
- [ ] 4.1 `buildLayout`: ensure the preview SVG's container is `position: relative` (class in
      `style.css`) and return it as `overlayContainer`.
- [ ] 4.2 `initApp`: `const cellEditor = buildCellEditor(overlayContainer, { onEdit: (baseId)
      => showShape(applyRampToShapeId(baseId, rampPanel.currentRamp()), { push: true }) });`
- [ ] 4.3 `showShape`: call `cellEditor.render(shapeId)` alongside `renderPreview` /
      `renderPrimitiveUsage`.
- [ ] 4.4 Initial-URL `decoded` branch and `popstate` handler: call `cellEditor.render`.

## 5. `web/src/style.css`

- [ ] 5.1 `.preview-container { position: relative }`; `.cell-overlay` (absolute grid);
...
- [ ] 6.1 `web/test/cell-editor.test.ts` (jsdom + vitest, pattern of `ramp-panel.test.ts`):
...
- [ ] 6.2 Ramp re-layering is covered by decomposition: `cell-editor` emits a base ID
      (6.1), and `shape-state.test.ts`'s `applyRampToShapeId` tests prove the `main.ts`
      one-liner re-applies the ramp.

## 7. Verification

- [ ] 7.1 `npm run test --workspace web` — all pass.
- [ ] 7.2 `npm run lint` and `npm run build` at the root pass.
...
- [ ] 7.4 Confirm Export SVG/PNG still produce the edited mark (overlay is a sibling of the
      SVG, not inside it).
